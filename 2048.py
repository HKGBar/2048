...
#   Only updating sprite obj on the screen is
#   a useful strategy for saving cpu power
...
BackGround = Background(
    resource_path("assets", "background.png"), [0, 0])
...
def init():
    ...
    for i in range(1, 12):
        ...
        image_file = resource_path("assets", str(pow(2, i)) + ".png")
        # graphic settings for image
    ...
    image_file = resource_path("assets", "0.png")
    tmpRawImg = pg.image.load(image_file).convert_alpha()
    tmpImg = pg.transform.smoothscale(tmpRawImg, (IMAGEWIDTH, IMAGEHEIGHT))
